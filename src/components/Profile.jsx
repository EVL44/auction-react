    ...
    const fetchData = async () => {
      try {
        ...
        let result = await fetch(
          `http://localhost:8000/api/user/${user.id}/products`
        );
        result = await result.json();
        setData(result);
        setLoading(false);
  ...
  async function handleSubmit(file) {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8000/api/upload-avatar/${user.id}?_method=PUT`,
        formData
      );
              ...
              <div className="info-det">
                ...
                <div className="edit-out-btns">
                  <Button
                    ...
                    className="font-c edit-btn"
                  >
                    <FaEdit /> Edit
                  </Button>
                  <Button className="font-c logout-btn" onClick={handleLogout}>
                  ...
                  <tbody>
                    {data.map((item) => (
                      <tr key={item.pid}>
                        ...
                        <td>
                          {getProductStatus(
                            new Date(item.expiration_time).getTime()
                          )}
                        </td>
                      </tr>
                    ))}
